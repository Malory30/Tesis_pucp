...
clear
cd "C:\Users\Malory\OneDrive\Documentos\NOVENO CICLO\TESIS\módulos 2016"
import dbase using "rech0"
...
clear
cd "C:\Users\Malory\OneDrive\Documentos\NOVENO CICLO\TESIS\módulos 2016"
import dbase using "ps_qaliwarma"
...
clear
cd "C:\Users\Malory\OneDrive\Documentos\NOVENO CICLO\TESIS\módulos 2016"
import dbase using "rec95"

save "control.dta", replace

*caseid
*S466B / control de crecimiento


save "establecimientos.dta",replace


*MODULO 73/ variables de violencia:

clear
cd "C:\Users\Malory\OneDrive\Documentos\NOVENO CICLO\TESIS\módulos 2016"
import dbase using "rec84dv"
...
*Módulo 66/ variables factor:


clear
cd "C:\Users\Malory\OneDrive\Documentos\NOVENO CICLO\TESIS\módulos 2016"
import dbase using "rec0111"
...
save "base_2016_final"

*gráficos:

clear
cd "C:\Users\Malory\OneDrive\Documentos\NOVENO CICLO\TESIS\módulos 2017"
use "base_2016_final.dta", clear



graph bar (percent) HV007 [pw=factorhogar], over(area) asyvars intensity(*.5) ///
 title("Area de residencia", size(medium)) ///
 ytitle("Porcentaje") ///
 blabel(bar, position(base) box bfcolor(white) size(large) format(%5.2f)) ///
 graphregion(color(white)) ylabel(,nogrid)
 
 
 graph bar (percent) HV007 [pw=factorhogar], over(i_riqueza) asyvars intensity(*.5) ///
 title("Indice de riqueza", size(medium)) ///
 ytitle("Porcentaje (%)") ///
 blabel(bar, position(outside) box bfcolor(white) size(large) format(%5.2f)) ///
 graphregion(color(white)) ylabel(,nogrid)
 
 
 graph bar (percent) HV007 [pw=factorhogar], over(educ_madre) asyvars intensity(*.5) ///
 title("Educación de la madre", size(medium)) ///
 ytitle("Porcentaje (%)") ///
 blabel(bar, position(outside) box bfcolor(white) size(large) format(%5.2f)) ///
 graphregion(color(white)) ylabel(,nogrid)
 
 graph bar (percent) HV007 [pw=factorhogar], over(serv_electr) asyvars intensity(*.5) ///
 title("Electricidad", size(medium)) ///
 ytitle("Porcentaje (%)") ///
 blabel(bar, position(outside) box bfcolor(white) size(large) format(%5.2f)) ///
 graphregion(color(white)) ylabel(,nogrid)
 
 graph bar (percent) HV007 [pw=factorhogar], over(sexo_niño) asyvars intensity(*.5) ///
 title("Sexo del niño", size(medium)) ///
 ytitle("Porcentaje (%)") ///
 blabel(bar, position(base) box bfcolor(white) size(large) format(%5.2f)) ///
 graphregion(color(white)) ylabel(,nogrid)
 
 graph bar (percent) HV007 [pw=factormujer], over(acceso_radio) asyvars intensity(*.5) ///
 title("Acceso a fuente de información: radio", size(medium)) ///
 ytitle("Porcentaje (%)") ///
 blabel(bar, position(outside) box bfcolor(white) size(large) format(%5.2f)) ///
 graphregion(color(white)) ylabel(,nogrid)
 
 graph bar (percent) HV007 [pw=factormujer], over(acceso_periodico) asyvars intensity(*.5) ///
 title("Frecuencia con la que mira la que lee periódico", size(medium)) ///
 ytitle("Porcentaje (%)") ///
 blabel(bar, position(base) box bfcolor(white) size(large) format(%5.2f)) ///
 graphregion(color(white)) ylabel(,nogrid)
 
 graph bar (percent) HV007 [pw=factormujer], over(acceso_televisión) asyvars intensity(*.5) ///
 title("Acceso a fuente de información: televisión", size(medium)) ///
 ytitle("Porcentaje (%)") ///
 blabel(bar, position(base) box bfcolor(white) size(large) format(%5.2f)) ///
 graphregion(color(white)) ylabel(,nogrid)
 
 graph bar (percent) HV007 [pw=factorhogar], over(serv_sanitario) asyvars intensity(*.5) ///
 title("Acceso a servicio sanitario", size(medium)) ///
 ytitle("Porcentaje (%)") ///
 blabel(bar, position(outside) box bfcolor(white) size(large) format(%5.2f)) ///
 graphregion(color(white)) ylabel(,nogrid)
 
 graph bar (percent) HV007 [pw=factorhogar], over(serv_agua, label(labgap(6))) asyvars intensity(*.5) ///
 title("Acceso a servicio agua", size(medium)) ///
 ytitle("Porcentaje (%)") ///
 blabel(bar, position(outside) box bfcolor(white) size(large) format(%5.2f)) ///
 graphregion(color(white)) ylabel(,nogrid)
 
 graph bar (percent) HV007 [pw=factorhogar], over(qaliwarma) asyvars intensity(*.5) ///
 title("Programa qaliwarma", size(medium)) ///
 ytitle("Porcentaje (%)") ///
 blabel(bar, position(outside) box bfcolor(white) size(large) format(%5.2f)) ///
 graphregion(color(white)) ylabel(,nogrid)
 
 graph bar (percent) HV007 [pw=factormujer], over(violencia_domestica) asyvars intensity(*.5) ///
 title("Violencia domestica", size(medium)) ///
 ytitle("Porcentaje") ///
 blabel(bar, position(base) box bfcolor(white) size(large) format(%5.2f)) ///
 graphregion(color(white)) ylabel(,nogrid)
 
 graph bar (percent) HV007 [pw=factorhogar], over(establecimiento_salud) asyvars intensity(*.5) ///
 title("Establecimientos de salud", size(medium)) ///
 ytitle("Porcentaje") ///
 blabel(bar, position(base) box bfcolor(white) size(large) format(%5.2f)) ///
 graphregion(color(white)) ylabel(,nogrid)

  
histogram zscore [iw=factorhogar] 
 
sum i_riqueza serv_electr acceso_radio acceso_televisión serv_sanitario serv_agua qaliwarma[pw=factorhogar] // utilizado en trabajo
